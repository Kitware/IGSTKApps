...
bool FiducialSet::LoadFiducialsFromXMLfile(QString fName, igstk::AxesObject::Pointer m_WorldReference)
{
  this->Clear();
  QFile file( fName );
  ...
  {
    QMessageBox::warning( m_Parent, "Loading", "Failed to import point set XML data." );
    file.close();
    return false;
  }
  ...
  {
    QMessageBox::warning( m_Parent, "Loading", "Invalid point set file." );
    return false;
  }
...
void FiducialSet::SetOpacity(double value)
{
  ...
  for (iFiducial=m_Plan.begin(); iFiducial != m_Plan.end(); iFiducial++)
  {
    m_AxialFiducialRepresentationVector[iFiducial->first]->SetOpacity(value);
    m_SagittalFiducialRepresentationVector[iFiducial->first]->SetOpacity(value);
    m_CoronalFiducialRepresentationVector[iFiducial->first]->SetOpacity(value);
